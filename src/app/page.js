...
export default function Home() {
  ...
  const isFolderMode = images.length > 0 && images[0].relativePath && images[0].relativePath.includes("/");
  ...
  const handleProcessBatch = async () => {
    ...
    try {
      for (let i = 0; i < total; i += batchSize) {
        ...
        const promises = batch.map(async (imageFile) => {
          try {
            ...
            let pathInZip = result.relativePath.replace(/^\/+/, "");
            zip.file(pathInZip, result.blobData);
          } catch (err) {
            console.error(`Error processing image ${imageFile.name}:`, err);
            zip.file(imageFile.relativePath || imageFile.name, imageFile);
          } finally {
            processed++;
      ...
      if (isFolderMode) {
        ...
        const firstPath = images[0].relativePath.replace(/^\/+/, "");
        const rootFolder = firstPath.split("/")[0];
        finalZipName = `${rootFolder} - con marca de agua`;
      ...
      alert("Hubo un error al procesar las imágenes. Revisa la consola.");
    } finally {
  ...
  const MAX_VISIBLE = 6;
  const stripStart = Math.max(0, Math.min(currentImageIndex - 2, images.length - MAX_VISIBLE));
      ...
      <header className="border-b border-zinc-900 bg-black sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-base font-bold tracking-tight select-none">WATERMARK STUDIO</span>
            <span className="text-[10px] font-mono text-zinc-500 border border-zinc-800 rounded-md px-1.5 py-0.5 select-none">v2</span>
          </div>
          ...
          <section className="lg:col-span-7 flex flex-col gap-5">
            ...
            {images.length === 0 ? (
              <div className="flex flex-col gap-4">
                <h1 className="text-xl font-bold tracking-tight select-none">Carga tus imágenes</h1>
                    ...
                    {stripSlice.map((file, relIdx) => {
                      ...
                      return (
                        <button
                          ...
                          title={file.name}
                          className={`cursor-pointer shrink-0 w-16 h-16 rounded-xl border-2 flex items-end p-1 overflow-hidden transition-all duration-200 ${isActive
                        ...
                        >
                          <div className="w-full h-full rounded-lg overflow-hidden relative">
                            ...
                            {isActive && <div className="absolute inset-0 ring-1 ring-inset ring-white/10 rounded-lg"></div>}
                          </div>
                        </button>
                      );
                    })}
